...
use \App\Transaction\Chaptercomment as Transaction_Chaptercomment;
use \Zx\View\View;
use \Zx\Test\Test;

class Chaptercomment extends Admin {

    public $list_page = '';
    public function init() {
        $this->view_path = APPLICATION_PATH . 'module/admin/view/chaptercomment/';
        $this->list_page =  ADMIN_HTML_ROOT . 'chaptercomment/retrieve/1/title/ASC/';
        \App\Transaction\Session::set_ck_upload_path('chaptercomment');
        parent::init();
    }
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($title <> '') {
                $arr = array('title' => $title, 
                    ...
                    'status'=>$status,
                    'cat_id' => $cat_id);
                if (Transaction_Chaptercomment::create_chaptercomment($arr)) {
                    $success = true;
                }
            }
        }
    ...
    public function delete() {
        $id = $this->params[0];
        Transaction_Chaptercomment::delete_chaptercomment($id);
        header('Location: ' . $this->list_page);
    }
    ...
    public function update() {
        ...
        if (isset($_POST['submit']) && isset($_POST['id'])) {
            ...
            if ($id <> 0) {
                ...
                if (Transaction_Chaptercomment::update_chaptercomment($id, $arr)) {
                    $success = true;
                }
            }
        }
        if ($success) {
            ...
            $chaptercomment = Model_Chaptercomment::get_one($id);
    ...
    public function retrieve() {
        ...
        \App\Transaction\Session::set_admin_current_l1_menu('Chaptercomment');
        ...
        $chaptercomment_list = Model_Chaptercomment::get_chaptercomments_by_page_num($where, $current_page, $order_by, $direction);
        $num_of_records = Model_Chaptercomment::get_num_of_chaptercomments($where);
        $num_of_pages = ceil($num_of_records / NUM_OF_RECORDS_IN_ADMIN_PAGE);
        //\Zx\Test\Test::object_log('chaptercomment_list', $chaptercomment_list, __FILE__, __LINE__, __CLASS__, __METHOD__);

        View::set_view_file($this->view_path . 'retrieve.php');
        View::set_action_var('chaptercomment_list', $chaptercomment_list);
