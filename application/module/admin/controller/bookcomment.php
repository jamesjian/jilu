...
use \App\Transaction\Bookcomment as Transaction_Bookcomment;
use \Zx\View\View;
use \Zx\Test\Test;

class Bookcomment extends Admin {

    public $list_page = '';
    public function init() {
        $this->view_path = APPLICATION_PATH . 'module/admin/view/bookcomment/';
        $this->list_page =  ADMIN_HTML_ROOT . 'bookcomment/retrieve/1/title/ASC/';
        \App\Transaction\Session::set_ck_upload_path('bookcomment');
        parent::init();
    }
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($title <> '') {
                $arr = array('title' => $title, 
                    ...
                    'status'=>$status,
                    'cat_id' => $cat_id);
                if (Transaction_Bookcomment::create_bookcomment($arr)) {
                    $success = true;
                }
            }
        }
    ...
    public function delete() {
        $id = $this->params[0];
        Transaction_Bookcomment::delete_bookcomment($id);
        header('Location: ' . $this->list_page);
    }
    ...
    public function update() {
        ...
        if (isset($_POST['submit']) && isset($_POST['id'])) {
            ...
            if ($id <> 0) {
                ...
                if (Transaction_Bookcomment::update_bookcomment($id, $arr)) {
                    $success = true;
                }
            }
        }
        if ($success) {
            ...
            $bookcomment = Model_Bookcomment::get_one($id);
    ...
    public function retrieve() {
        ...
        \App\Transaction\Session::set_admin_current_l1_menu('Bookcomment');
        ...
        $bookcomment_list = Model_Bookcomment::get_bookcomments_by_page_num($where, $current_page, $order_by, $direction);
        $num_of_records = Model_Bookcomment::get_num_of_bookcomments($where);
        $num_of_pages = ceil($num_of_records / NUM_OF_RECORDS_IN_ADMIN_PAGE);
        //\Zx\Test\Test::object_log('bookcomment_list', $bookcomment_list, __FILE__, __LINE__, __CLASS__, __METHOD__);

        View::set_view_file($this->view_path . 'retrieve.php');
        View::set_action_var('bookcomment_list', $bookcomment_list);
